...
import { boardType, stringObjectType } from '../../types/types';
...
const Content = ({ data }: { data: boardType }) => {
  const gameImg: stringObjectType = {
    Maze: maze,
    Running: running,
  };
  ...
  return (
    <div css={style.content}>
      <img src={gameImg[category]} alt="img" css={style.img} />
      <div css={style.contentInfo}>
        ...
        {content.split('$').map((text, idx) => (
          <div css={style.description} key={idx}>
            {text}
          </div>
        ))}
      </div>
      <button
        ...
        type="button"
        onClick={handleLike}
        css={style.likeBtn(isLiked)}></button>
    </div>
  );
};
